...
from .input_callbacks_space_phy2 import input_event_callback
...
def get_valid_position_for_obj(obj: GObject):
    objs = []
    for obj_id, obj in gamectx.object_manager.get_objects().items():
        objs.append(obj)
...
def add_asteroid():
    o = GObject(Body(mass=50, moment=0,body_type=Body.STATIC), depth=0)
...
def default_collision_callback(arbiter: pymunk.Arbiter, space, data):
    ...
    for s in arbiter.shapes:
        s: Shape = s
        o = gamectx.object_manager.get_by_id(s.get_object_id())
        if o is None:
            return False
